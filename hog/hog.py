...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    total_points, sow_sad, roll = 0, 0, 0
    while num_rolls >= 1:
        roll = dice()
        if roll == 1:
            sow_sad = 1
...
def piggy_points(score):
    ...
    "*** YOUR CODE HERE ***"
    square = score * score
    lowest_value = square % 10 
    square //= 10
    while square > 0:
        if square % 10 < lowest_value:
            lowest_value = square % 10
        square //= 10
    return lowest_value + 3
...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    def first_two_digits(score):
        while score >= 100:
            score //= 10
        return score
    player_score = first_two_digits(player_score)
    opponent_score = first_two_digits(opponent_score)
    if (player_score % 10) < (opponent_score % 10) and (player_score // 10) < (opponent_score // 10):
        return True
    else: 
        return False
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            dice_roll = strategy0(score0, score1)
            score0 += take_turn(dice_roll, score1, dice, goal)
            if more_boar(score0, score1) == False:
                who = 1
            else: 
                who = 0
        else:
            dice_roll = strategy1(score1, score0)
            score1 += take_turn(dice_roll, score0, dice, goal)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        runninghigh = running_high
        if who == 0:
            if (score0 > lastscore) and ((score0 - lastscore) > runninghigh):
                print('Player', who, 'has reached a new maximum point gain.', score0 - lastscore, 'point(s)!')
                runninghigh = score0 - lastscore
                lastscore = score0
            else: 
                lastscore = score0
        if who == 1:
            if (score1 > lastscore) and ((score1 - lastscore) > runninghigh):
                print('Player', who, 'has reached a new maximum point gain.', score1 - lastscore, 'point(s)!')
                runninghigh = score1 - lastscore
                lastscore = score1
            else: lastscore = score1
        return announce_highest(who, lastscore, runninghigh)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max = 1
    while trial_max < 11:
        dice_roll = make_averaged(roll_dice)(trial_max, dice)
        if dice_roll > max:
            max = dice_roll
            max_roll = trial_max
        trial_max += 1
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if piggy_points(opponent_score) >= cutoff or more_boar(score + piggy_points(opponent_score), opponent_score):
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
